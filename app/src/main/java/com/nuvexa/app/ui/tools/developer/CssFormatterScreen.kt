...
import com.nuvexa.app.core.util.formatCss
...
@Composable
fun CssFormatterScreen(modifier: Modifier = Modifier) {
    ...
    Column(modifier = modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(spacing.l)) {
        NuvexaTextField(value = input, onValueChange = { input = it }, label = stringResource(R.string.tool_dev_css_formatter_name), minLines = 6)
        PrimaryButton(
            text = stringResource(R.string.action_format),
            onClick = { output = formatCss(input) },
