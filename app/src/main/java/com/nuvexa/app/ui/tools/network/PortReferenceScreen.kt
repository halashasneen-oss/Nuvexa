...
fun PortReferenceScreen(modifier: Modifier = Modifier) {
    ...
    val filtered = remember(query) {
        if (query.isBlank()) {
            ...
            COMMON_PORTS.filter {
                it.port.toString().contains(query) || it.service.contains(query, ignoreCase = true)
            }
        }
    }
        ...
        filtered.forEach { entry ->
            Card(
                ...
                Column(modifier = Modifier.padding(spacing.m)) {
                    Text("${entry.port} — ${entry.service}", style = NuvexaExtraType.numericMedium)
