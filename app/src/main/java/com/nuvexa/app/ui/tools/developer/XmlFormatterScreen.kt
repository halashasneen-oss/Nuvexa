...
import com.nuvexa.app.core.util.formatXml
...
@Composable
fun XmlFormatterScreen(modifier: Modifier = Modifier) {
    ...
    fun format() {
        formatXml(input).onSuccess { output = it; error = null }.onFailure {
            output = ""
    ...
    Column(modifier = modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(spacing.l)) {
        NuvexaTextField(value = input, onValueChange = { input = it }, label = stringResource(R.string.tool_dev_xml_formatter_name), minLines = 6)
        PrimaryButton(text = stringResource(R.string.action_format), onClick = ::format, modifier = Modifier.fillMaxWidth(), enabled = input.isNotBlank())
